function plotInCanvas(data,str){
	...
	for(var i=0;i<data.length;i++)
		if(absmax<Math.abs(data[i].y))
			absmax = Math.abs(data[i].y);
	//quater points without scaling
	var quater = [[0,0],[0,0]];
	for(var i=0,j=0;i<data.length&&j<2;i++){
		if(Math.abs(data[i].y)==absmax){
			quater[j][0]=data[i].x;
			quater[j][1]=-data[i].y;
			j++;
		}
	...
	if(quater[0][1]==0&&quater[1][1]==0){
		...
		quater[0][0] = data[Math.floor(data.length/4)].x;
		quater[1][0] = data[Math.floor(data.length/4*3)].x;
		quater[0][1] = -0.001;
		quater[1][1] = -quater[0][1];
	}
		
	//scale parameter
	//scale with the minimum, maximum and half of the whole range
	...
	//first point
	var first = [data[0].x*width_scale,data[0].y*height_scale];
	//middle point [0,0]
	var middle = [data[Math.floor(data.length/2)].x*width_scale,data[Math.floor(data.length/2)].y*height_scale];
	//last point
	var last = [data[data.length-1].x*width_scale,data[data.length-1].y*height_scale];
	...
	//plot with scaling
	ctx.beginPath();
	ctx.moveTo(data[0].x*width_scale,-data[0].y*height_scale);
	for(var i=1;i<data.length;i++)
		ctx.lineTo(data[i].x*width_scale,-data[i].y*height_scale);
	ctx.stroke();
}